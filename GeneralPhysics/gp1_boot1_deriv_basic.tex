...
\Remark{However, this ``$\sin^kx$'' notation is used only if $k$ is a positive integer ($k\ge 2$). For negative power or fractional power, we \Emph{always} write as $(\sin x)^k$:
\begin{equation*}
  (\sin x)^{-2}=\frac{1}{\sin^2x},\qquad(\tan x)^{-1}=\frac1{\tan x}=\cot x,\qquad (\cos x)^{1/2}=\sqrt{\cos x},\quad\text{etc.}
\end{equation*}
(You will soon learn that $\sin^{-1}x$ etc.~are used to express different functions.)
}
