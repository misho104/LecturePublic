...
\begin{quizzes}
  ...
  \TAB\QSub{1} What does $|\vc a|$ mean?\qquad\QSub{2} What does $|k|$ mean?\\
  \TAB\hfill\Hint{They are different! The answer can be found in these two pages.}
\end{quizzes}
...
\begin{problems}
  ...
  \begin{enumerate}
    ...
    \item Explain why $|x\vc a+y\vc b|^2$ is \Emph{not} equal to $x^2+y^2$.
...
Therefore, \Emph{we cannot do any other operations}. For example, these are all \Emph{invalid} expression:
{
\color{AltDefA}
\begin{align*}
   (\vc a)^2,
