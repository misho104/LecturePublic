...
\makeatletter
\begingroup
\let\pgfsyspdfmark\@gobblethree
\let\@writefile\@gobbletwo
\input{gp1_boot1_deriv_basic.aux}
\endgroup
\makeatother
...
Recall Eqs.~\eqref{eq:diff}--\eqref{eq:diffdefex} in the basic part. We saw that derivatives are \Emph{defined by} Eq.~\eqref{eq:diff} and thus., for example,
\begin{equation}
...
\smallskip
  ...
  \item Now, prove Eqs.~\eqref{eq:reci} and \eqref{eq:prod}. \Hint{This is not easy.}
\end{enumerate} 
\end{enumerate}
