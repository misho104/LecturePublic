...
\begin{enumerate}
  ...
  \begin{itemize}
    \item If $b>0$, then the exponential $b^x$ is well-defined for any $x\in\RR$, i.e., $x$ can be positive, negative, fractional, or zero.
    \item If $b<0$, then $b^x$ is well-defined only for an integer $x\in\ZZ$.\addnote{%
In fact, $b^x$ can be defined as a real number if $b<0$ and $x$ is a rational number with an odd denominator, such as $(-2)^{1/7}=-1.10409$, but we rarely see (and maybe should not use) such expressions.
    }\addnote{In \emph{complex analysis}, we may define $b^x=(\ee^{\ln b})^x =\ee^{x\ln b}$ for $b<0$. However, $\ln b$ is multi-valued for $b<0$ and may be a complex number. For example, we can check that
    $z=1+\ii\sqrt{3}$, $z=1-\ii\sqrt{3}$, and $z=-2$ all satisfy $z^3=-8$, so we can say any of them is $(-8)^{1/3}$.}
    \item If $b=0$, then $b^x=0^x$ is well-defined only for $x>0$ (and its value is always zero).\addnote{%
There is no good way to define $0^x$ for $x\le 0$. For example, the value of $0^0$ is not well-defined. We sometimes use $0^0\stackrel!=1$ but it is not a global consensus.}
  \end{itemize}
...
\Emph{One} solution is $f(x)=2x^2$. However, later in the problems, you will see that there are many more solutions.
