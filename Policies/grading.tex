...
\hypersetup{
  ...
  pdfcopyright={2023–2026 Sho Iwamoto},
...
\begin{miniitemize}
  \item $\mathbfup{A+}$ for ``performance that has gone beyond Sho's expectation'',
...
\begin{itemize}
  ...
  \item Sho wants students to be independent thinkers, rather than relying on the ``truth'' given by authority.
        They are asked to find the best solution by themselves, not to reach the same solution as Sho's.
\end{itemize}
Sho also thinks that returning answer sheets will have no effect on improving their learning, while it consumes a significant amount of Sho's time, which should be used for other educational activities.
\begin{itemize}
...
\begin{itemize}
  \item Mini tests. Sho designs them as an opportunity to provide feedback, rather than as a means to evaluate achievement, so that students can recognize their shortcomings in discussion and writing.
  ...
  \item During office hours. However, only if students rework an exam or test and produce their own revised answers. They need to have in-person visits.
\end{itemize}
...
\begin{tabbing}
  ...
  \JA{△}\>: Sho thinks it is worth partial mark (with a number $n$ $\to$ $n$\JA{折})\addnote{For $1\le n\le 9$, $\text{(full mark)}\times0.1n$. For $10\le n\le99$, $\text{(full mark)}\times0.01n$.}.\\[.5em]
