...
#let make-entries(
  ...
  if rest.len() > 0 {
    ...
    let searchFunc = p => {
      (
        p.index-type == "Range" and p.rangeEnd == page-link.page
          or p.index-type == indextype.Cardinal and p.page == page-link.page and p.fmt == page-link.fmt
      )
    }
  ...
  for indexed in query(<jkrb_index>) {
    ...
    let entries = entry
      ...
      .map(e => {
        ...
        (display: disp, key: k, render: render)
      })
...
#let render-entry(idx, entry, lvl, use-page-counter, sort-order, entry-casing, range-delimiter, spc, mpc) = {
  ...
  let render = entry.at("render", default: auto)

  let rendered-pages = {
    let p = pages.map(if render == auto { render-link.with(use-page-counter, range-delimiter, spc, mpc)} else { render })
    box(width: lvl * 1em)
    apply-entry-casing(
      display,
      entry-casing,
